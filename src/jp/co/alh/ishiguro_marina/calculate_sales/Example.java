...
//3.集計
	...
	//連番処理
			...
			for	(int i = 0 ; i < rcdlist.size(); i++){
				...
				String sales;
					...
					if(!branchSaleMap.containsKey(branchcode)) {
						System.out.println(rcdlist.get(i).getName()+"の支店コードが不正で----す");
						return;
					}
					if(!commoditySaleMap.containsKey(commoditycode)) {
						System.out.println(rcdlist.get(i).getName()+"の商品コードが不正で----す");
						return;
					}
		...
		//マップ代入
					...
					if(totalbranch >= 1000000000 || totalcommodity >= 1000000000){
